...
- **Credential-less boot caveat (Node 22 + firebase-admin):** with no Firebase Admin credentials (no `FIREBASE_SERVICE_ACCOUNT` / ADC — the default in this VM), the server boots and starts serving, but ~5s later a background Firestore hydration step (`hydrate*FromDurableStore`) triggers an *unhandled* ADC rejection (`Could not load the default credentials`) that Node 22 treats as fatal, killing the process. Run dev/start with `NODE_OPTIONS="--unhandled-rejections=warn" npm run dev` so the process survives. The resulting `[Firebase Admin] ... Could not load the default credentials` / `MetadataLookupWarning` lines are expected and harmless — Firestore is optional (write-through has a local-file fallback), and the SPA + APIs work fully. In real Cloud Run, ADC is present so this flag is unnecessary. The same applies to `npm start` when run without credentials.
