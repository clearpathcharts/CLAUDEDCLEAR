  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col gap-2 p-2 lg:grid lg:grid-cols-[220px_minmax(0,1fr)_280px] lg:grid-rows-[minmax(420px,1fr)_220px] lg:gap-2">
      <aside className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-[#FF1493]/30 bg-black/70 lg:row-span-2">
        <h2 className="border-b border-white/10 px-3 py-2 text-base font-black uppercase tracking-[0.16em] text-[#FF1493]">
          Watchlist
        </h2>
        <ul className="min-h-0 flex-1 overflow-y-auto font-mono text-base font-bold">
          {quotes.map((row) => {
            ...
            return (
              <li key={row.symbol}>
                ...
                >
                  <span>
                    <span className="block font-extrabold text-white">{row.symbol}</span>
                    <span className="block text-sm font-bold uppercase text-zinc-500">{row.latency}</span>
                  </span>
                  <span className="text-right">
                    {row.live && row.price !== null ? (
                      <>
                        <span className="block font-bold tabular-nums text-zinc-100">
                          {formatStructurePrice(row.price)}
                        </span>
                        {row.pct !== null && (
                          <span
                            className={`block text-sm font-bold ${row.pct >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}
                          >
                            {row.pct >= 0 ? '+' : ''}
                            {row.pct.toFixed(2)}%
                          </span>
                      ...
                      <span className="text-sm font-bold uppercase text-zinc-600">No quote</span>
                    )}
                  </span>
                </button>
              </li>
      ...
      <section className="flex min-h-[420px] min-w-0 flex-col overflow-hidden rounded-xl border border-white/15 bg-black/80">
        <div className="space-y-2 border-b border-white/10 px-3 py-2">
          <div className="flex flex-wrap items-end justify-between gap-2">
            ...
            {report && (
              <dl className="flex flex-wrap gap-3 font-mono text-sm font-bold uppercase text-zinc-400">
                <div>
            ...
            {TIMEFRAMES.map((tf) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={on}
                  className="rounded border px-2 py-1 text-sm font-extrabold uppercase tracking-widest"
                  style={{
                    color: on ? '#fff' : '#a1a1aa',
      ...
      <aside className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-[#FF1493]/25 bg-black/70">
        ...
        <p className="px-3 py-1.5 font-mono text-sm font-bold uppercase leading-relaxed text-zinc-500">
          Derived from the loaded OHLC series. Not signals.
        </p>
        <ul className="min-h-0 flex-1 overflow-y-auto">
          {recentEvents.length === 0 && (
            <li className="px-3 py-4 font-mono text-sm font-bold uppercase text-zinc-600">
          ...
          {recentEvents.map((ev, i) => (
            ...
            >
              <span className={`text-sm font-extrabold uppercase tracking-wide ${familyTone(ev.family)}`}>
                {ev.family}
              </span>
              <span className="min-w-0 truncate font-mono text-sm font-bold text-zinc-400">{ev.label}</span>
        ...
        <div className="border-t border-white/10 px-3 py-2">
          <p className="mb-1 font-mono text-sm font-bold uppercase text-zinc-500">Engine roster</p>
          <ul className="space-y-0.5">
            {InstitutionalRegistry.map((item) => (
              <li key={item.id} className="font-mono text-sm font-bold text-zinc-500">
      ...
      <section className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-black/70">
        <h2 className="border-b border-white/10 px-3 py-2 text-base font-black uppercase tracking-[0.16em] text-zinc-300">
          Volume profile
        </h2>
        <p className="px-3 py-1 font-mono text-sm font-bold uppercase text-zinc-500">
      ...
      <section className="grid min-h-0 grid-cols-1 gap-2 sm:grid-cols-2">
        <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-black/70">
          ...
          <div className="flex flex-1 flex-col justify-center px-3 py-3">
            {lastCvdPoint ? (
              <>
                <p className="font-mono text-3xl font-extrabold tabular-nums text-white">
                  {lastCvdPoint.cumulativeDelta.toLocaleString()}
                </p>
                <p className="mt-1 font-mono text-sm font-bold uppercase text-zinc-500">
                  Last bar delta {lastCvdPoint.delta.toLocaleString()} ·{' '}
                  {report?.volumeMode === 'vendor' ? 'vendor volume' : 'range proxy'}
                </p>
              </>
            ) : (
              <p className="font-mono text-sm font-bold uppercase text-zinc-600">No CVD until candles load</p>
            )}
          </div>
        </div>
        <div className="flex min-h-0 flex-col overflow-hidden rounded-xl border border-white/10 bg-black/70">
          ...
          {newsError && (
            <p className="px-3 py-2 font-mono text-sm font-bold text-rose-400">{newsError}</p>
          )}
          <ul className="min-h-0 flex-1 overflow-y-auto">
            {news.map((item, i) => (
              <li key={`${item.title}-${i}`} className="border-b border-white/5 px-3 py-1.5">
                {item.link ? (
                  <a
                    ...
                    className="text-base font-bold leading-snug text-zinc-200 hover:text-[#00FFFF]"
                  >
                    {item.title}
                  </a>
                ) : (
                  <span className="text-base font-bold leading-snug text-zinc-200">{item.title}</span>
                )}
                <span className="mt-0.5 block font-mono text-sm font-bold uppercase text-zinc-600">
                  {item.source}
                </span>
              </li>
            ))}
