...
export default function CpmsApk() {
  ...
  const isUserAuthorized = () => {
    ...
    if (typeof localStorage !== 'undefined' && localStorage.getItem('cpms_cabinet_admin') === 'true') return true;
      ...
      <div className="relative z-10 max-w-7xl mx-auto space-y-10">
        ...
        <header className="flex flex-col md:flex-row items-center justify-between gap-6 border-b border-white/[0.04] pb-6">
          ...
          <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
            ...
            {/* ADMINISTRATION CABINET SWITCH */}
            <button
              onClick={() => {
                setIsCabinetOpen(!isCabinetOpen);
                if (!isCabinetOpen && isUserAuthorized()) {
                  setAdminBypass(true);
                }
              }}
        ...
        {/* --- COLLAPSIBLE MEDIA CABINET (ADMIN SECTION) --- */}
        <AnimatePresence>
          {isCabinetOpen && (
            <motion.div
            ...
            >
              <div className="p-6 space-y-6">
                <div className="flex items-center justify-between border-b border-white/5 pb-4">
                  ...
                  {/* SECURITY BYPASS KEY TRAP */}
                  <div className="flex items-center gap-3 bg-black/40 p-2 rounded-full border border-white/5 select-none">
                    <span className="text-[10px] font-mono text-zinc-400 font-bold px-3">
                      {isUserAuthorized() ? 'AUTHORIZED ✓' : 'LOCKED ACCESS'}
                    </span>
                    <button
                      onClick={() => {
                        const next = !adminBypass;
                        setAdminBypass(next);
                        localStorage.setItem('cpms_cabinet_admin', next ? 'true' : 'false');
                      }}
                      className={`px-3 py-1.5 rounded-full text-[9px] font-mono font-black transition-all cursor-pointer ${
                        adminBypass 
                          ? 'bg-amber-500 text-black shadow-[0_0_10px_rgba(245,158,11,0.3)]' 
                          : 'bg-zinc-800 text-zinc-400 hover:bg-zinc-700'
                      }`}
                    >
                      {adminBypass ? 'REVOKE PRIVILEGES' : 'BYPASS KEY'}
                    </button>
                  </div>
                </div>
                ...
                {!isUserAuthorized() ? (
                  <div className="py-12 text-center space-y-3 flex flex-col items-center justify-center">
                    ...
                    <h4 className="text-sm font-cinzel font-bold text-red-400 uppercase">ACCESS FORBIDDEN</h4>
                    <p className="text-xs font-mono text-zinc-400 max-w-md font-semibold leading-relaxed">
                      Your session lacks catalog modification parameters. Push the **BYPASS KEY** switch above to bypass client-side and upload streams freely.
                    </p>
                  </div>
                ) : (
