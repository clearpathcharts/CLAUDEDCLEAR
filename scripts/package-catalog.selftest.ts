/**
 * Product packages: four locked membership packages + add/remove add-ons.
 * No list prices. Run: npx tsx scripts/package-catalog.selftest.ts
 */
...
const created = addPackage({
  ...
  summary: 'Custom chart reminders — no list price.',
...
assert.equal(listPackages().length, 5);
...
assert.doesNotMatch(panel, /8\.99|49\.99|89\.99|Checkout/);
